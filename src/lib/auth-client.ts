 ...
 * Uses VITE_CONVEX_SITE_URL with window.location.origin as fallback
 * This ensures auth works on deployed sites even if the env var is missing
 */
...
export async function signInWithEmail(
  ...
  try {
    ...
    if (error) {
      console.error("[FlipDash Auth] Sign-in error:", { code: error.code, message: error.message });
      return {
        success: false,
        error: {
    ...
    console.error("[FlipDash Auth] Sign-in exception:", err);
    return {
      success: false,
      error: {
...
export async function signUpWithEmail(
  ...
  try {
    ...
    if (error) {
      console.error("[FlipDash Auth] Sign-up error:", { code: error.code, message: error.message });
      return {
        success: false,
        error: {
          message: getErrorMessage(error.code),
    ...
    console.error("[FlipDash Auth] Sign-up exception:", err);
    return {
      success: false,
      error: {
        message: err instanceof Error ? err.message : "Connection failed. Please try again.",
...
export async function signOutUser(): Promise<SignInResult> {
  try {
    ...
    console.error("[FlipDash Auth] Sign-out exception:", err);
    return {
      success: false,
      error: {
